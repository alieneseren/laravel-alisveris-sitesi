...
@section('title', 'Kart Bilgileri - Stripe Ödeme')
            ...
            <div class="row">
                ...
                <div class="col-md-5">
                    ...
                    <div class="card shadow-sm mb-3">
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex justify-content-between align-items-center py-1 border-bottom">
                                <div>
                                    <small class="fw-bold">{{ $item['urun']->ad }}</small>
                                    <br>
                                    <small class="text-muted">{{ $item['miktar'] }} adet × {{ number_format($item['urun']->fiyat, 2) }} ₺</small>
                                </div>
                    ...
                    <div class="card shadow-sm mb-3">
                        ...
                        <div class="card-body">
                            <strong>{{ $addressData['ad_soyad'] }}</strong><br>
                            <small class="text-muted">{{ $addressData['telefon'] }}</small><br>
                            <small class="text-muted">{{ $addressData['email'] }}</small><br>
                            <hr class="my-2">
                            <small>
                                {{ $addressData['adres'] }}<br>
                                {{ $addressData['ilce'] }} / {{ $addressData['il'] }}
                                @if($addressData['posta_kodu'])
                                    <br>{{ $addressData['posta_kodu'] }}
                                @endif
                            </small>
                            <hr class="my-2">
                            <a href="{{ route('stripe.checkout.form') }}" class="btn btn-outline-primary btn-sm">
                            ...
                            <form id="payment-form">
                                ...
                                <div class="d-grid gap-2 mt-4">
                                    <button type="submit" class="btn btn-success btn-lg py-3" id="submit-button">
                                        ...
                                        <span id="button-text">{{ number_format($toplam, 2) }} ₺ Öde</span>
                                        <div id="spinner" class="spinner-border spinner-border-sm ms-2 d-none" role="status">
                                            <span class="visually-hidden">Loading...</span>
                                        </div>
                                    </button>
                                    
                                    <a href="{{ route('stripe.checkout.form') }}" class="btn btn-outline-secondary">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        try {
            ...
            const response = await fetch('{{ route("stripe.create.payment.intent") }}', {
                ...
                body: JSON.stringify({
                    ...
                    customer_data: @json($addressData)
                })
            });
            ...
            const {error, paymentIntent} = await stripe.confirmCardPayment(
                paymentData.client_secret,
                {
                    payment_method: {
                        card: cardElement,
                        billing_details: {
                            name: '{{ $addressData["ad_soyad"] }}',
                            email: '{{ $addressData["email"] }}',
                            phone: '{{ $addressData["telefon"] }}',
                            address: {
                                line1: '{{ $addressData["adres"] }}',
                                city: '{{ $addressData["il"] }}',
                                state: '{{ $addressData["ilce"] }}',
                                postal_code: '{{ $addressData["posta_kodu"] ?? "" }}',
                                country: 'TR'
                            }
                        }
                    }
    ...
    function setLoading(loading) {
        ...
        if (loading) {
            ...
            buttonText.textContent = '{{ number_format($toplam, 2) }} ₺ Öde';
